...
class APIService: NSObject {
    ...
    // MARK: - Set the URL Session for Provinces
        func getAvailableProvinces(iso: String, completion: @escaping (Result<[[String: AnyObject]]>) -> Void){
            ...
            // 3: Set configuration type to a singleton shared session and dataTask
            ...
            let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in
                if (error != nil) {
                    ...
                    // 4: JSONSerialization for data from API
                    do {
                        if let json = try JSONSerialization.jsonObject(with: data, options: [.mutableContainers]) as? [String: AnyObject] {
    ...
    // MARK: - Set the URL Session for Regions
        func getAvailableRegions(completion: @escaping (Result<[[String: AnyObject]]>) -> Void){
            ...
            // 2: Set request API with httpMethod "GET"
